"""
Disease-weighted loss function for autoencoder training (CROPPED VERSION).

Prioritizes reconstruction of diseased regions based on color deviation from healthy tissue.

MODIFIED: Does not zero out weights for non-leaf pixels. All pixels within the cropped
bounding box contribute to the loss, with disease-based weighting applied uniformly.
"""
...
class DiseaseWeightedLoss(nn.Module):
    """
    Weighted MSE loss that prioritizes diseased regions (CROPPED VERSION).
    ...
    MODIFIED: All pixels in the cropped region contribute to loss (not just masked pixels).
    """
    ...
    def compute_disease_weights(self, lab_target, mask, lab_mean, lab_std):
        ...
        # MODIFIED: Don't multiply by mask - keep weights for all pixels
        # Previously: weights = weights * mask (which zeroed out non-leaf pixels)
        # Now: All pixels within cropped region contribute to loss with disease-based weights

        return weights

    def forward(self, predictions, targets, masks, embeddings=None, lab_stats=None):
        """
        Compute disease-weighted reconstruction loss (CROPPED VERSION).

        Args:
            predictions: (B, 3, H, W) - Predicted LAB image
            targets: (B, 3, H, W) - Target LAB image
            masks: (B, 1, H, W) - Binary mask (kept for compatibility but not used to zero weights)
        ...
        """
        # Compute disease weights (now applied to all pixels, not just masked ones)
        if lab_stats is not None:
            ...
            # Fallback to uniform weights for ALL pixels
            weights = torch.ones_like(masks)

        # MODIFIED: Weighted MSE loss on ALL pixels in cropped region
...
class SimpleMSELoss(nn.Module):
    """Simple MSE loss for all pixels in cropped region (CROPPED VERSION)."""

    def __init__(self):
        super(SimpleMSELoss, self).__init__()

    def forward(self, predictions, targets, masks, embeddings=None, lab_stats=None):
        """Compute simple MSE loss on ALL pixels (not just masked ones)."""
        # MODIFIED: MSE on all pixels in cropped region
        squared_error = (predictions - targets) ** 2

        # Normalize by total number of pixels
        num_pixels = squared_error[0, 0].numel()  # H * W
