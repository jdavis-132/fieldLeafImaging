...
trait_groups <- c('sam3_covar')
for(grp in trait_groups)
{
  summary <- read_csv(str_c('output/gwas/mlm_20260324/sam3/covar', '/GWAS_summary_by_traits_methods.csv'))
  pmap_files <- Sys.glob(str_c('output/gwas/mlm_20260324/sam3/covar', '/GWAS_*_all_results.csv'))
  
  for(f in pmap_files)
  {
    trait <- str_remove(f, str_c('output/gwas/mlm_20260324/sam3/covar', '/GWAS_')) %>% 
    ...
    manhattan <- plotManhattan(pmap, 
                               ...
                               species = 'sorghum')
    ggsave(str_c('output/gwas/mlm_20260324/sam3/covar', '/', trait, '_manhattan.png'), 
           plot = manhattan, 
           width = 5, 
           height = 2.5, 
           dpi = 300, 
    ...
    write_csv(sig_markers, str_c('output/gwas/mlm_20260324/sam3/covar', trait, '_significant_markers.csv'))
  }
}
