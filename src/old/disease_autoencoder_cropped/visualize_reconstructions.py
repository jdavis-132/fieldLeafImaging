...
Usage:
    python -m disease_autoencoder_cropped.visualize_reconstructions
    python -m disease_autoencoder_cropped.visualize_reconstructions --num_samples 16
    python -m disease_autoencoder_cropped.visualize_reconstructions --split test
    python -m disease_autoencoder_cropped.visualize_reconstructions --split train --num_samples 12
"""
...
from src.disease_autoencoder_cropped.model import DiseaseAutoencoder
from src.disease_autoencoder_cropped.dataset import DiseaseLeafDataset
...
def create_reconstruction_grid(model, dataset, lab_stats, config, num_samples=8, save_path=None):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices):
            ...
            title = f"{genotype_short}\nDev {metadata['device']} | MSE: {mse.mean().item():.5f}"
...
def create_detailed_comparison(model, dataset, lab_stats, config, num_samples=4, save_path=None):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices):
            ...
            axes[i, 3].set_title(f'Error Heatmap\nL:{L_mae.mean().item():.3f} a:{a_mae.mean().item():.3f} b:{b_mae.mean().item():.3f}',
                                fontsize=10)
            axes[i, 3].axis('off')
...
def create_best_worst_comparison(model, dataset, lab_stats, config, save_path=None):
    ...
    with torch.no_grad():
        for idx in range(len(dataset)):
            ...
            mse = ((target - reconstruction) ** 2) #* sample['mask']).sum() / sample['mask'].sum()
            mse_list.append((idx, mse.mean().item()))
