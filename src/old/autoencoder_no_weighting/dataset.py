"""
Dataset for standard autoencoder with LAB color space preprocessing.

Note: This dataset is identical to the disease-weighted version - the only difference
is in the loss function used during training. The data loading and preprocessing
remains the same.
"""
...
class DiseaseLeafDataset(Dataset):
    """
    Dataset for disease-aware autoencoder (CROPPED VERSION).
    ...
    5. Normalize LAB using pixel statistics
    6. MODIFIED: Keep original pixel values for non-leaf areas (don't zero them out)
    """
    ...
    def _compute_lab_statistics(self):
        """Compute mean and std of LAB channels using efficient batch computation."""
        print("Computing LAB statistics from all images...")

        # Compute statistics in two passes (more accurate and efficient)
        ...
        for idx in range(len(self.image_metadata)):
            ...
            sum_vals += lab_image.sum(axis=(0, 1))
            n_pixels += lab_image.shape[0] * lab_image.shape[1]

            if (idx + 1) % 1000 == 0:
                print(f"  Computing mean: {idx + 1}/{len(self.image_metadata)} images...")
        ...
        for idx in range(len(self.image_metadata)):
            ...
            diff = lab_image - mean
            sum_squared_diff += (diff ** 2).sum(axis=(0, 1))

            if (idx + 1) % 1000 == 0:
                print(f"  Computing std: {idx + 1}/{len(self.image_metadata)} images...")

        # Compute std
        std = np.sqrt(sum_squared_diff / n_pixels)
    ...
    def __getitem__(self, idx):
        ...
        # MODIFIED: Keep original pixel values for non-leaf areas
        # Previously: lab_image = lab_image * mask_3ch (which set background to 0)
        # Now: Keep all pixel values as-is after normalization
...
def get_dataloaders(splits, config):
    ...
    """
    # Load LAB statistics from shared file (disease_autoencoder_cropped)
    import json
    shared_stats_path = Path(__file__).parent.parent / 'disease_autoencoder_cropped' / 'logs' / 'lab_statistics.json'
    with open(shared_stats_path, 'r') as f:
        lab_stats = json.load(f)
    print(f"Loaded shared LAB statistics from {shared_stats_path}")
    print(f"  LAB mean: {lab_stats['lab_mean']}")
    print(f"  LAB std: {lab_stats['lab_std']}")

    # Create training dataset without computing statistics
    ...
    train_dataset.set_lab_statistics(lab_stats['lab_mean'], lab_stats['lab_std'])
    ...
    train_dataset_aug.set_lab_statistics(lab_stats['lab_mean'], lab_stats['lab_std'])
