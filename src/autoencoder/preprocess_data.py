...
import sys
import csv
sys.path.insert(0, '/home/schnable/Documents/fieldLeafImaging')
...
def main():
    ...
    parser.add_argument('--use_mask', '-m', default=False, help='Use masked images in training (default: False)')
    ...
    for idx, image in enumerate(raw_images):
        ...
        # save mask to mask dir
        sam3_results = segmenter.segment_image(image)

        # Skip if no objects detected and save to list
        if len(sam3_results['masks']) == 0:
        ...
        combined_mask = combine_masks(sam3_results['masks'], save_path=mask_path)
        ...
        print(f"  Found {len(sam3_results['masks'])} objects, mask saved")
